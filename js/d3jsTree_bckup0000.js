...
//});
    ...
    var tree = d3.layout.tree()
        .size([viewerHeight, viewerWidth]).nodeSize([350, 40])
        .separation(function(a, b) { return a.parent === b.parent ? 3.5 : .5; });
    ...
    //.on("mouseover", function(d){return tooltip.style("visibility", "visible");tooltip.select('.name').html(d.name)})
...
if (d.source.name == "Statics") 
    d3.select(this).remove();});
    
    var couplingParent1 = tree.nodes(root).filter(function(d) {
            return d['name'] === 'Topic 8';
        })[0];
	var couplingChild1 = tree.nodes(root).filter(function(d) {
            return d['name'] === 'Point Forces';
        })[0];
